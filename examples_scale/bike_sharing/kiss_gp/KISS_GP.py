...
if __name__ == '__main__':
    logging.info(f'----------------------------------------')
    logging.info(f'Training KISS_GP with Bike Sharing')
    ...
    gp = KISS_GP(train_x, train_y)

    model, _ = gp.train_data(train_x, train_y)
    mean, lower, upper = gp.infer(test_x, test_y)

    plot_mean_variance(test_y.numpy()[:100], mean.numpy()[:100], saveFile='KISS_GP_mean_variance')
